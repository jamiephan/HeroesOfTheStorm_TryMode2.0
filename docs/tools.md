...
To do so, add the behavior to the selected units (e.g using the chat command [`addbehavior MZeratulCleave`](./usage/LibUnits.md#-command-addbehavior))
...
This will generate a unit with the prefixed `UN`. For instance, the model `RetreatPing` (White Flag retreat ping model) will have a unit `UNRetreatPing`, You can summon this unit using the chat command: [`summon UNRetreatPing`](./usage/LibUnits.md#-command-summon).
