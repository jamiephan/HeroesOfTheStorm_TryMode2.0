...
/**
 * Function to generate a galaxy template
...
function generate(libname, varname, des) {
  ...
  console.log('');
  console.log(
    `Then, in the main Library file (${libname}.galaxy), locate the ${libname}_InitTriggers() function and add this trigger init function:`,
  );
  console.log('');
  ...
  console.log(
    `        if (!(((StringWord(EventChatMessage(false), 1) == "${varname}") || (StringWord(EventChatMessage(false), 1) == "${varname}")))) {`,
  );
