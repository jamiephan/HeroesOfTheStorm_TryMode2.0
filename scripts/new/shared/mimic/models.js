...
export default () => {
  ...
  heroesFileExtract.queue(/^mods.*\.xml$/, (extractedFiles) => {
    ...
    finalXML +=
      `<!-- To prevent collision, all Mimic Models Unit's id will have prefixed "${ID_PREFIX}" -->\n`;
    finalXML +=
    ...
    finalXML += `${TAB(0)}<!-- Dummy Parent Unit for mimic model to inherit from -->\n`;
      ...
      XMLParser(obj.fileData, (err, result) => {
        ...
        Object.keys(catalog).forEach((k) => {
          ...
          catalog[k].forEach((a) => {
            ...
            MODELS.push(id);
            console.log(`Mapped Model ${id} to Unit ${ID_PREFIX}${id} in file ${obj.fileName}.`);
          });
        });
        finalXML += `${TAB(1)}<!-- ///////////////////////////////////-->\n`;
